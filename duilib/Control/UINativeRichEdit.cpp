...
// 		m_RichEditCtrlTS.SetDefaultCharFormat(cf);
	...
	bool CNativeRichEditUI::CanPaste()
	{
		...
		if(m_pSkinScrollFrame)
		{
			...
			LPTSTR    lptstr;
			CDuiString sTest;
			for(TIntToStrMap::iterator it1 = mpFormat.begin(); it1 != mpFormat.end(); ++it1)
			{
				hglb = GetClipboardData(it1->first); 
				if (hglb != NULL) 
				{ 
	...
	void CNativeRichEditUI::InsertText(LPCTSTR sText)
	{
		...
		if(m_pSkinScrollFrame)
		{
			HWND hRichEdit = ((CSkinScrollFrame*)m_pSkinScrollFrame)->GetNativeRichEditCtrl();
			if(hRichEdit)
				::SendMessage(hRichEdit, EM_REPLACESEL, TRUE, (LPARAM)sText);//从当前位置插入文字;
		}
	}

	void CNativeRichEditUI::_MoveCaretToEnd(HWND hRE)
	{
		::SendMessage(hRE, EM_SETSEL, 0, (LPARAM)-1);          // Select the entire contents.
		CHARRANGE chrg;//cp:Characer Position;
		SendMessage(hRE, EM_EXGETSEL, 0, (LPARAM)&chrg);  // Get the selection into a CHARRANGE.
	}
	...
	BOOL CNativeRichEditUI::InsertBitmap(HBITMAP hBitmap, int iDesiredPixelX, int iDesiredPixelY, DWORD dwUserTag)
	{
		HWND hRichEdit = NULL;
		if(m_pSkinScrollFrame)
			hRichEdit = ((CSkinScrollFrame*)m_pSkinScrollFrame)->GetNativeRichEditCtrl();
	...
	BOOL CNativeRichEditUI::InsertImage(LPCTSTR szImagePath, int iDesiredPixelBox, DWORD dwUserTag)
	{
		...
		BOOL bSuc = InsertBitmap(pImgInfo->hBitmap, szOut.cx, szOut.cy, dwUserTag);
	...
	//注意:有可能导出某个图片失败, 即ppExportFile的某个元素为NULL!
	BOOL CNativeRichEditUI::ExportPicture(LPCTSTR szDir, int& iExportCount, int*& pPos, LPTSTR*& ppExportFile, DWORD*& pUsrTag)
	{
		//jian.he test begin
		//先测试一下  AnsiToUnicode() AnsiToUnicode()
		//std
		//jian.he test end
		...
		//Get OLE Interface;
		IRichEditOle* pReOle = nullptr;
		SendMessage(hRichEdit, EM_GETOLEINTERFACE, 0, (LPARAM)&pReOle);
		if(!pReOle)
			return FALSE;

		//CLSID_Picture_Dib
		const long iReObjCount = pReOle->GetObjectCount();
		if(iReObjCount < 1)
			return TRUE;
		...
		//allocate memory;
		pPos = new int[iReObjCount];
		memset(pPos, 0, iReObjCount*sizeof(int));
		ppExportFile = new LPTSTR[iReObjCount];
		memset(ppExportFile, 0, iReObjCount*sizeof(LPTSTR));
		pUsrTag = new DWORD[iReObjCount];
		memset(pUsrTag, 0, iReObjCount*sizeof(DWORD));

		for(int i = 0; i < iReObjCount; ++i)
		{
			REOBJECT reObj = { 0 };
			...
			if(S_OK == pReOle->GetObject(i, &reObj, REO_GETOBJ_POLEOBJ))
			{
				if(reObj.poleobj)
				{
					...
					if(reObj.dwUser > 0)//caller自己插入的,caller已经掌握了文件位置;
						continue;

					//if((reobject.dwFlags & REO_SELECTED) == REO_SELECTED) //判断reObj是否处于选中状态;				

					IDataObject* pDataObject = NULL;
					if(S_OK == reObj.poleobj->QueryInterface(IID_IDataObject, (void**)&pDataObject))
					{
						//IDataObject IStorage有什么关系?
						STGMEDIUM stgMdm;
						FORMATETC fmtEtc;
						//fmtEtc.cfFormat = CF_BITMAP;        // Clipboard format = CF_BITMAP //HERE,IT CAN'T BE SURE!
						fmtEtc.ptd = NULL;                  // Target Device = Screen
						fmtEtc.dwAspect = DVASPECT_CONTENT; // Level of detail = Full content
						fmtEtc.lindex = -1;                 // Index = Not applicaple
						fmtEtc.tymed = TYMED_GDI;           // 对应CF_BITMAP

						//reObj的数据格式可能有多种可能: CF_BITMAP CF_METAFILEPICT CF_DIB等,因为不确定,所以需要循环可能的集合:
						IEnumFORMATETC *pEnumFmt = NULL;
						if(S_OK != pDataObject->EnumFormatEtc(DATADIR_GET, &pEnumFmt))
							continue;

						//##遍历获取一个可用的剪切板格式##BEGIN;
						HRESULT hrNextCF = S_OK;
						ULONG Fetched = 0;
						while(S_OK == hrNextCF)
						{
							hrNextCF = pEnumFmt->Next(1, &fmtEtc, &Fetched);
							if(SUCCEEDED(hrNextCF))
							{
								//目前支持的格式: CF_METAFILEPICT CF_BITMAP CF_DIB
								//CF_METAFILEPICT:Word复制过来的图片一般是这种格式;
								//CF_BITMAP:一般OLE插入以CF_BITMAP方式插入;
								//不支持hEnhMetaFile;
								bool bOk = false;
								if((CF_METAFILEPICT == fmtEtc.cfFormat) || (CF_BITMAP == fmtEtc.cfFormat) || (CF_DIB == fmtEtc.cfFormat))//
								{
									HRESULT hr = pDataObject->GetData(&fmtEtc, &stgMdm);
									if((S_OK == hr) && stgMdm.hBitmap)
									{
										if((CF_BITMAP == fmtEtc.cfFormat) || (CF_DIB == fmtEtc.cfFormat))
										{
											if(stgMdm.hBitmap)
												bOk = SaveImage(szDir, stgMdm.hBitmap);
										}
										else if(CF_METAFILEPICT == fmtEtc.cfFormat)
										{
// 											if(stgMdm.hMetaFilePict)
// 											{
// 												LPMETAFILEPICT pMFP = (LPMETAFILEPICT)::GlobalLock(stgMdm.hMetaFilePict);
// 												bOk = SaveToBitMap(pMFP, strPath);
// 												::GlobalUnlock(stgMdm.hMetaFilePict);
// 											}
										}
										else if(CF_ENHMETAFILE == fmtEtc.cfFormat)
										{
											if(stgMdm.hEnhMetaFile)
											{
												//待实现!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
												//bOk = 
											}
									...
									::ReleaseStgMedium(&stgMdm);//需要考虑pUnkForRelease吗?MSDN说的不清楚!
								}
								if(bOk)
									break;
							}
				...
				//如果使用REO_GETOBJ_ALL_INTERFACES,那么以下代码也需要执行;
				//if(reObj.pstg)
				//{
				//	//可以把stream写入文件吗?
				//	reObj.pstg->Release();
	...
	bool CNativeRichEditUI::SaveImage(LPCTSTR szDir, HBITMAP hBmp)
	{
		bool bSuc = false;
		...
		if(pBmp)
		{
			...
			std::wstring wsDir;
#ifndef _UNICODE
			AnsiToUnicode(szDir, wsDir);
#else
			wsDir = szDir;
#endif
			time_t iNow = time(0);
			wchar_t szNow[50] = {0};
			wprintf(szNow, L"%I64d", iNow);
			wsDir += szNow;
			bSuc = (Gdiplus::Status::Ok == pBmp->Save(wsDir.c_str(), &bmpClsid));
	...
	bool CNativeRichEditUI::AnsiToUnicode(LPCSTR asStr, std::wstring& wsStr)
	{
		if(!asStr || strlen(asStr) < 1)
			return false;

		int iLenNeed = ::MultiByteToWideChar(CP_ACP, 0, asStr, -1, NULL, 0);
		iLenNeed += 1; //wcslen(szT) + 1;会出现末尾的乱码! 如果是UTF8可能3个字节表示一个字符呢!
		WCHAR* szW = new WCHAR[iLenNeed];
		wmemset(szW, 0, iLenNeed);
		::MultiByteToWideChar(CP_ACP, 0, asStr, -1, szW, iLenNeed);
		wsStr = szW;
		delete szW;
		return true;
	}

	bool CNativeRichEditUI::Utf8ToUnicode(std::string& aStr, std::wstring& wStr)
	{
		if (aStr.size() < 1)
			return false;
	...
	BOOL CNativeRichEditUI::Test_CopyAsRTF()
	{
		...
		if (OpenClipboard(hRichEdit)) 
		{
			...
			if(hglbCopy)
			{
				...
				//GlobalFree(hglbCopy);//剪切板中的数据本身就是全局内存中的数据,所以肯定是不能释放的,让RE自己控制.
				//hglbCopy = NULL;
				CloseClipboard();
				bRet = TRUE;
			}
...
#define PIXEL_PER_INCH (fResW/fDevW*25.4f)
		...
		rcFmtRange.right = 1000.0f / PIXEL_PER_INCH*1440;//fClientW  jian.he testing testing!!!!应该获取ScrollInfo计算如果RichEdit平铺开占据的Width/Height;
		rcFmtRange.bottom = 5000.0f / PIXEL_PER_INCH * 1440;//fClientH

		Gdiplus::Bitmap* pBmpSnap = new Gdiplus::Bitmap(1000, 5000);
		Gdiplus::Graphics* gSnap = Gdiplus::Graphics::FromImage(pBmpSnap);
		HDC hdcSnap = gSnap->GetHDC();
		

		FORMATRANGE fmtRange;
		fmtRange.hdc = hdcSnap;// ::GetDC(hRichEdit);
		fmtRange.hdcTarget = hdcSnap;//hdcMem; TCanvas.Handle
		...
		//{
			fmtRange.chrg.cpMax = -1;// iCharsCount;
			...
			gSnap->ReleaseHDC(hdcSnap);//TRIGGER SAVE OPERATION;

			CLSID bmpClsid;
			GetEncoderClsid(L"image/png", &bmpClsid);
			Gdiplus::Status st1 =  pBmpSnap->Save(L"RESnap.png", &bmpClsid);
	...
	void CNativeRichEditUI::Test_TraverseRE()
	{
		HWND hRichEdit = NULL;
		if(m_pSkinScrollFrame)
			hRichEdit = ((CSkinScrollFrame*)m_pSkinScrollFrame)->GetNativeRichEditCtrl();
	...
	void CNativeRichEditUI::Test_GetScrollInfo()
	{
		HWND hRichEdit = NULL;
		if(m_pSkinScrollFrame)
			hRichEdit = ((CSkinScrollFrame*)m_pSkinScrollFrame)->GetNativeRichEditCtrl();
		...
		//(TEST IF THE SCROLLINFO UDPATED OR NOT WHEN RE IS INVISIBLE)
		SCROLLINFO si;
		memset(&si, 0, sizeof(si));
		si.cbSize = sizeof(si);
		si.fMask = SIF_ALL;
		::GetScrollInfo(hRichEdit, SB_HORZ, &si);

		memset(&si, 0, sizeof(si));
		si.cbSize = sizeof(si);
		si.fMask = SIF_ALL;
		::GetScrollInfo(hRichEdit, SB_VERT, &si);
	}
#endif
