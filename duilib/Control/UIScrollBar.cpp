...
namespace DuiLib
{
	...
	{
		//sif( m_bHorizontal == bHorizontal ) return;//因为默认值的关系, 会造成: 当bHorizontal与默认值相等时,下面的代码永远得不到执行!jian.he 2018/03/08;
	...
	int CScrollBarUI::GetThumbWidth()
	{
		int iW = DEFAULT_SCROLLBAR_SIZE;
		RECT rcThumbW = m_thumbNormalImage.GetSource();
		if(rcThumbW.right - rcThumbW.left > 0)
			iW = rcThumbW.right - rcThumbW.left;
