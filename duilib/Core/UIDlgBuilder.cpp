                                   ...
                                   CPaintManagerUI* pManager, CControlUI* pParent)
{
	BYTE* pByte = NULL;
	DWORD dwSize = 0;
	BOOL bParseXmlOver = false;//jian.he added 2018/03/06;
...
// 		return NULL;
	...
	if (pByte)
	{
		if(m_xml.LoadFromMem(pByte, dwSize))
			bParseXmlOver = true;
		delete[] pByte;
	}
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
		...
		//树控件XML解析
            ...
            case 14:
				...
				else if( _tcscmp(pstrClass, DUI_CTR_NATIVERICHEDIT) == 0 )    pControl = new CNativeRichEditUI;
                break;
            case 15:
